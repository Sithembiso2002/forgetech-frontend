      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-8 mt-12 md:mt-16">
          {reasons.map((item, i) => (
            <div
              key={item.title}
              className="group relative bg-white rounded-2xl p-6 md:p-8 shadow-sm hover:shadow-xl transition-all duration-300 border border-neutral-border/30 hover:border-brand-tech/20 flex flex-col items-start gap-4"
            >
              {/* Icon */}
              <span className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-brand-gold/10 text-brand-gold group-hover:bg-brand-gold group-hover:text-white transition-colors duration-300">
